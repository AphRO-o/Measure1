...
6.本软件为求高复用性，未将最终各点高程纳入计算，原因是每两点间的测站数量皆不尽相同，如果逐一输入，反倒不如最后自行计算高程来得高效，因此表格的高程一列需要用户手动输入。
...
### 本软件为求高复用性，未将最终各点高程纳入计算，原因是每两点间的测站数量皆不尽相同，如果逐一输入，反倒不如最后自行计算高程来得高效，因此表格的高程一列为手动输入列。
